...
async function generateRustSmartContract(userTask) {
  try {
    ...
    console.log("Dependencies solved!")
    
    // Generar el código Rust
    rustPrompt = generateRustPrompt(userTask,id);
    let rustCode = await generateMessageWithClaude(rustPrompt, model);
    
    // Guardar en la carpeta original
    const rustFilePath = path.join(projectDir, '../programs/kobe-solana/src/lib.rs');
    fs.writeFileSync(rustFilePath, rustCode);
    
    // Guardar también en la carpeta de Stylus
    const stylusFilePath = path.join(projectDir, '../../stylusBranch/src/lib.rs');
    const stylusCode = transformToStylusFormat(rustCode);
    fs.writeFileSync(stylusFilePath, stylusCode);
    
    console.log(`Code written to ${rustFilePath} and ${stylusFilePath}`);
...
function transformToStylusFormat(solanaCode) {
